...
const MainLayout = () => {
  ...
  const handleLogOut = () => {
    logOut()
      .then(() => {
        axiosSecure
          .post(`${mainUrl}/api/v1/logout`, { user: user?.email })
          .then((res) => {
            console.log(res);
            setLoading(false);
            navigate("/");
        ...
        <NavLink
          className={({ isActive, isPending }) =>
            isPending
              ? "pending"
              : isActive
              ? "border-b-2 rounded-none border-[#209CEE]"
              : "containers"
          }
          to="/"
        >
                  ...
                  {user ? (
                    <div className="relative">
                      <img
                        ...
                        className="rounded-full cursor-pointer outline-[#209CEE] outline-4 outline  md:w-8 w-8 md:h-8 h-10 object-cover"
                        src={user.photoURL}
                        alt=""
                      />
